...
namespace Geometry {
    ...
    struct Point {
        ...
        std::pair<double, double> to_pair() {
    ...
    bool are_perpendicular(Vector2 A, const Vector2 B) {
        return fequal(A.dot(B), 0);
    }
    ...
    bool is_convex(const vector<Point> &polygon) {
        ...
        bool sample = CCW(polygon[0], polygon[1], polygon[2]);
        for (int current = 0; current < num_points; current++) {
            int next = (current+1) % num_points;
            int later = (current+2) % num_points;

            if (CCW(polygon[current], polygon[next], polygon[later]) == sample) continue;
            return false;
        }
